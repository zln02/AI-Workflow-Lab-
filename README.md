...
> **Status:** Archived (개발 종료)
> **개발 기간:** 2026-01-22 ~ 2026-03-16
> **종료 사유:** 계획 종료 — 학습·탐색 목적 달성 후 마감
> **라이브 데모:** 없음. 현재 가동 중인 서버·도메인 없음
> **재현:** Tomcat 9 + MySQL 8 수동 구성 ([설치 및 실행](#설치-및-실행))

이 저장소는 더 이상 개발·운영되지 않는다. 아래 문서는 개발 당시의 설계와 구현을
기록으로 남긴 것이다. 개발 기간은 사람이 작성한 커밋 41건의 최초·최종 시점을
실측한 값이다.

> **배포 이력에 대하여**: 이 저장소에는 **실제 배포·운영 기록이 남아 있지 않다.**
> `docs/DEPLOYMENT.md` 는 새 서버에 올리는 방법을 적은 일반 가이드이고,
> GitHub Actions 실행 이력은 0건이며, 배포된 도메인·서버를 가리키는 기록도 없다.
> 따라서 이 문서에는 운영 기간이나 가동 실적을 적지 않는다.
