...
<%
  ...
  try {
    ...
    String status = json.containsKey("status") ? (String) json.get("status") : "pending";
    String notes = json.containsKey("notes") ? (String) json.get("notes") : "";
    double amount = json.containsKey("amount") ? ((Number) json.get("amount")).doubleValue() : 0.0;

    // 주문 업데이트 로직 (현재는 성공 응답만 반환)
